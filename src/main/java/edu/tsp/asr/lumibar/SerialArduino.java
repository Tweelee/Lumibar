...
public class SerialArduino implements SerialPortEventListener {
    ...
    public synchronized void write(String str) {
        if (serialPort != null && output != null) {
            try {
                output.write(str);
                output.flush();
                System.out.println("Serial write: "+str);
            } catch (Exception e) {
    ...
    /** method added by Marie-Pierre
    * same method as write but with char (because RVB need only to be between 0-255 : one byte)
         */
    public synchronized void writeChar(char c) {
        if (serialPort != null && output != null) {
            try {
                output.write(c);
                output.flush();
                System.out.println("Serial write: "+c);
            } catch (Exception e) {
                System.err.println("Write error: "+e.toString());
            }
        }
    ...
    public synchronized void serialEvent(SerialPortEvent oEvent) {
        if (oEvent.getEventType() == SerialPortEvent.DATA_AVAILABLE) {
            try {
                ...
                System.out.println("Serial read: "+inputLine);
            } catch (Exception e) {
